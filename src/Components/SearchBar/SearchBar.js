...
import {  Image, StyleSheet, Text, TextInput, View, VirtualizedList } from 'react-native'
...
const SearchBar = () => {
  ...
  const FetchResult =useCallback(async(searchData)=>{
      if(searchData){
        ...
        const Response = await axios (`https://api.themoviedb.org/3/search/movie?api_key=ee685f440549ded82e3e87a8eed2f321&query=${searchData}`)
      ...
      <Card>
       <View style={styles.containerList}>
        <Image
          ...
          source={{ uri: poster_path ? image500(poster_path) : 'placeholder_image_url' }}
          resizeMode="cover"
        />
        <View style={styles.textSection}>
          ...
          <View style={styles.buttonSection}>
            <Button title="Favorite" onPress={() => console.log('Favorite clicked')}>
              <FontAwesome name="bookmark-o" size={30} color="white" />
            </Button>
            <Button title="Heart" onPress={() => console.log('Heart clicked')}>
